...
| S-19 | CodeQL's Rust analysis is currently unusable — 14 alerts on the first run, all false, including 3 "critical". Dismissed individually with reasons (see `.github/workflows/README.md`). Re-evaluate when the Rust support matures; do not switch it off | low | |
| S-18 | ~~`tofv doctor` printed the gateway host and username in clear~~ — fixed in 0.1.3. That output is what people paste into bug reports, and the log was already redacted while the doctor was not. Host and username are hidden by default; `--show-profile` reveals them. Port and whether a realm is set are still shown, since those are what actually help diagnose | medium | |
| S-17 | ~~Nothing reported which TOFV was installed~~ — fixed in 0.1.2: `tofv doctor` and the panel show the app version, and the helper line reports the **installed** helper's version and flags a mismatch. Found the hard way: a helper from an earlier build ran for weeks and only a manual checksum comparison surfaced it | medium | |
